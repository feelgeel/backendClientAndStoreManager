...
const {ManifactureOwner}=require("../models/manifactureOwner_Modal")
const schema = {
...
// console.log(userBody);
  ...
  //   if(respon.errors) return res.status(400).send("something is wrong");
    let newproduct=new ManifactureOwner(userBody);
...
router.get("/", async(req, res) => {
  const Product=await ManifactureOwner.find().limit(5).catch(err=>{});
  res.send(Product)
});
router.get("/:id", async(req, res) => {
  let id=req.params.id;
  const Product=await ManifactureOwner.find({userId:id}).catch(err=>{});
  res.send(Product)
});
...
router.get("/gting/:gting", async(req, res) => {
  ...
  const Product=await ManifactureOwner.find({Gting:id}).catch(err=>{});
  res.send(Product)
  console.log(id)
});
