...
const {SupplierOwner}=require("../models/supplierOwner_model")
const schema = {
...
// console.log(userBody);
  ...
  //   if(respon.errors) return res.status(400).send("something is wrong");
    let newproduct=new SupplierOwner(userBody);
...
router.get("/", async(req, res) => {
  const Product=await SupplierOwner.find().limit(5).catch(err=>{});
  res.send(Product)
});
router.get("/:id", async(req, res) => {
  let id=req.params.id;
  const Product=await SupplierOwner.find({_id:id}).catch(err=>{});
  res.send(Product)
});
router.get("/userId/:userId", async(req, res) => {
  let id=req.params.userId;
  const Product=await SupplierOwner.find({userId:id}).catch(err=>{});
  res.send(Product)
});
router.get("/gting/:gting", async(req, res) => {
  let id=req.params.gting;
  const Product=await SupplierOwner.find({Gting:id}).catch(err=>{});
  res.send(Product)
  console.log(id)
});
